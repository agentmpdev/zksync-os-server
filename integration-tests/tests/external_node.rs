...
use zksync_os_integration_tests::{
    Tester, assert_traits::ReceiptAssert, contracts::EventEmitter, integration_test_matrix,
};
use zksync_os_server::config::Config;

integration_test_matrix!(
    #[test_log::test(tokio::test)]
    batch_verification_works,
    |case| async move {
        let builder = case.builder().batch_verification(1);
...
integration_test_matrix!(
    #[test_log::test(tokio::test)]
    batch_verification_without_enough_ens,
    |case| async move {
        let builder = case.builder().batch_verification(2);
        let main_node = builder.build().await?;
...
integration_test_matrix!(
    ...
    transaction_replay,
    |case| async move {
        let main_node = case.setup().await?;
