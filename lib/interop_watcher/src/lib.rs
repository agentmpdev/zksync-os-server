...
impl L1InteropRootsWatcher {
    ...
    async fn fetch_events(
        ...
        to_block: u64,
        start_log_index: u64,
    ) -> anyhow::Result<Vec<(InteropRoot, u64, u64)>> {
        ...
        for log in logs {
            ...
            interop_roots.push((interop_root, log_block_number, log_index_in_block));

            self.next_log_to_scan_from = InteropRootsLogIndex {
                block_number: log_block_number,
                log_index: log_index_in_block + 1,
            };
    ...
    async fn poll(&mut self) -> anyhow::Result<()> {
        ...
        let interop_roots = self
            .fetch_events(
                self.next_log_to_scan_from.block_number,
                latest_block,
                self.next_log_to_scan_from.log_index,
            )
            .await?;
        ...
        for interop_root in interop_roots {
            let index = InteropRootsLogIndex {
                block_number: interop_root.1,
                log_index: interop_root.2,
            };
            let interop_root_envelope =
                InteropRootsEnvelope::from_interop_root(interop_root.0, index);
